...
get_link_git_replace() {
  
  print_banner
  printf "${WHITE} 💻 Insira o link do GITHUB para alterar da instalacao atual:${GRAY_LIGHT}"
  printf "\n\n"
  read -p "> " link_git_replace
}
...
software_update_replace() {
  get_link_git_replace
  get_empresa_atualizar
  frontend_update_replace
  backend_update_replace
}
...
inquiry_options() {
  ...
  printf "   [6] Alter. GitHub e Atualizar Canal Vem Fazer\n"
  printf "\n"
  read -p "> " option
